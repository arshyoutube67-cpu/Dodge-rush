// Dodge Rush - single-file gameplay
(() => {
  ...
  // Player
  const player = {
    x: W/2, y: H/2, r: 12, color: '#7ad6ff',
    vx:0, vy:0, maxSpeed: 750, // px/s
    follow: false, targetX: W/2, targetY: H/2
  };
  ...
  window.addEventListener('pointerup', () => player.follow = false);
  ...
  function spawnFromEdge(edge, speed, aim, r=14){
    ...
    // direction toward aim with slight randomness
  ...
  function restart(){
    ...
    running = true;
    // quick spawn to restart intensity
    spawnTimer = 300;
  }
  ...
  function update(dt){
    if (gameOver) return;
    difficultyTime += dt;
    // Spawn interval shrinks over time to make it harder
    ...
    for (let i = obstacles.length - 1; i >= 0; i--){
      ...
      o.x += (o.vx || 0) * dt/1000;
      o.y += (o.vy || 0) * dt/1000;
      // difficulty increases: slightly speed up
      if (o.speed) {
      ...
      // near-miss detection (risky close call)
      const nearThreshold = player.r + o.r + 18; // slightly larger than collision
  ...
  function draw(){
    // screen clear with subtle gradient
    ...
    // small HUD extras: timer bar showing difficulty
    const barW = 140, barH = 6;
    ctx.fillStyle = 'rgba(255,255,255,0.06)';
    ctx.fillRect(W/2 - barW/2, 8, barW, barH);
    const p = clamp((difficultyTime*0.0006)%1, 0, 1); // not very meaningful, just motion
  ...
  // Initialize
  highEl.textContent = 'HI ' + high;
  // initial warm obstacles
  for (let i=0;i<4;i++) spawnPattern();
  requestAnimationFrame(loop);

  // Restart on tap overlay
  overlay.addEventListener('pointerdown', (e) => {
    if (gameOver) restart();
  });

  // quick trick: allow click to nudge spawn (for fun)
  canvas.addEventListener('click', (e) => {
    // small boost if playing
    if (!gameOver) {
      // spawn a quick wave toward cursor for risk
      const rect = canvas.getBoundingClientRect();
